...
Record the flame amount belong to 


### constructor

```solidity
constructor(IERC20 _FLAME, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) public
```
...
| ---- | ---- | ----------- |
| _FLAME | IERC20 | The FLAME token contract address. |
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| flamePerSecond | uint256 | Flame reward per second. |
...
```solidity
function add(uint256 _allocPoint, IDonaswapV3Pool _v3Pool, bool _withUpdate) external
```
...
| _v3Pool | IDonaswapV3Pool | Address of the V3 pool. |
...
```solidity
event AddPool(uint256 pid, uint256 allocPoint, IDonaswapV3Pool v3Pool, ILMPool lmPool)
```
