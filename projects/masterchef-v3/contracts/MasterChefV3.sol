...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    uint256 public latestPeriodFlamePerSecond;
    ...
    /// @notice Record the flame amount belong to MasterChefV3.
    uint256 public flameAmountBelongToMC;
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, IDonaswapV3Pool indexed v3Pool, ILMPool indexed lmPool);
    ...
    constructor(IERC20 _FLAME, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        FLAME = _FLAME;
        nonfungiblePositionManager = _nonfungiblePositionManager;
        WETH = _WETH;
    }

    /// @notice Returns the flame per second , period end time.
    /// @param _pid The pool pid.
    /// @return flamePerSecond Flame reward per second.
    ...
    function getLatestPeriodInfoByPid(uint256 _pid) public view returns (uint256 flamePerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            flamePerSecond = (latestPeriodFlamePerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice Returns the flame per second , period end time. This is for liquidity mining pool.
    ...
    function getLatestPeriodInfo(address _v3Pool) public view returns (uint256 flamePerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            flamePerSecond =
                (latestPeriodFlamePerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) /
                totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice View function for checking pending FLAME rewards.
    /// @dev The pending flame amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (FLAME.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    function add(uint256 _allocPoint, IDonaswapV3Pool _v3Pool, bool _withUpdate) external onlyOwner {
    ...
    /// @notice Update the given pool's FLAME allocation point. Can only be called by the owner.
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotDonaswapNFT();
    ...
    /// @notice harvest flame from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return reward Flame reward.
    ...
    function transferToken(address _token, address _to) internal {
        ...
        // Need to reduce flameAmountBelongToMC.
        if (_token == address(FLAME)) {
            unchecked {
                // In fact balance should always be greater than or equal to flameAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balance >= flameAmountBelongToMC) {
                    balance -= flameAmountBelongToMC;
                } else {
                    // This should never happend.
                    flameAmountBelongToMC = balance;
                    balance = 0;
                }
            }
        }
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant {
        ...
        // Need to reduce flameAmountBelongToMC.
        if (token == address(FLAME)) {
            unchecked {
                // In fact balance should always be greater than or equal to flameAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balanceToken >= flameAmountBelongToMC) {
                    balanceToken -= flameAmountBelongToMC;
                } else {
                    // This should never happend.
                    flameAmountBelongToMC = balanceToken;
                    balanceToken = 0;
                }
            }
        }
    ...
    /// @param _amount The amount of flame injected.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        // Transfer flame token from receiver.
        FLAME.safeTransferFrom(receiver, address(this), _amount);
        // Update flameAmountBelongToMC
        unchecked {
            flameAmountBelongToMC += _amount;
        }
        ...
        uint256 flamePerSecond;
        uint256 flameAmount = _amount;
        if (latestPeriodEndTime > currentTime) {
            uint256 remainingFlame = ((latestPeriodEndTime - currentTime) * latestPeriodFlamePerSecond) / PRECISION;
            emit UpdateUpkeepPeriod(latestPeriodNumber, latestPeriodEndTime, currentTime, remainingFlame);
            flameAmount += remainingFlame;
        }
        flamePerSecond = (flameAmount * PRECISION) / duration;
        unchecked {
            latestPeriodNumber++;
            latestPeriodStartTime = currentTime + 1;
            latestPeriodEndTime = endTime;
            latestPeriodFlamePerSecond = flamePerSecond;
        }
        emit NewUpkeepPeriod(latestPeriodNumber, currentTime + 1, endTime, flamePerSecond, flameAmount);
    }

    /// @notice Update flame reward for all the liquidity mining pool.
    ...
    /// @notice Update flame reward for the liquidity mining pool.
    ...
    /// @notice Safe Transfer FLAME.
    /// @param _to The FLAME receiver address.
    /// @param _amount Transfer FLAME amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = FLAME.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            // Update flameAmountBelongToMC
            unchecked {
                if (flameAmountBelongToMC >= _amount) {
                    flameAmountBelongToMC -= _amount;
                } else {
                    flameAmountBelongToMC = balance - _amount;
                }
            }
            FLAME.safeTransfer(_to, _amount);
        }
    }
