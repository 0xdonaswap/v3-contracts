...
contract MasterChefV3Receiver is Ownable {
    ...
    /// @param _flame Flame token address.
    /// @param _v2Pid The pool id of the dummy pool on the MCV2.
    constructor(IMasterChefV2 _v2, IMasterChefV3 _v3, IERC20 _flame, uint256 _v2Pid) {
        MasterChefV2 = _v2;
        MasterChefV3 = _v3;
        Flame = _flame;
        V2_Pid = _v2Pid;

        Flame.safeApprove(address(_v3), type(uint256).max);
    }
    ...
    /// @notice Harvest pending FLAME tokens from MasterChef V2.
    ...
    /// @notice Withdraw unexpected tokens sent to the receiver, can also withdraw flame.
    /// @dev Callable by owner.
