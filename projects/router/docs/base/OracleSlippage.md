...
```solidity
function getBlockStartingAndCurrentTick(contract IDonaswapV3Pool pool) internal view returns (int24 blockStartingTick, int24 currentTick)
```
...
```solidity
function getPoolAddress(address tokenA, address tokenB, uint24 fee) internal view virtual returns (contract IDonaswapV3Pool pool)
```
