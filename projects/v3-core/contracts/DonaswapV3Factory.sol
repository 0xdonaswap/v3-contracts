...
import './interfaces/IDonaswapV3Factory.sol';
import "./interfaces/IDonaswapV3PoolDeployer.sol";
import './interfaces/IDonaswapV3Pool.sol';

/// @title Canonical Donaswa V3 factory
/// @notice Deploys Donaswap V3 pools and manages ownership and control over pool protocol fees
contract DonaswapV3Factory is IDonaswapV3Factory {
    /// @inheritdoc IDonaswapV3Factory
    ...
    function createPool(
        ...
        pool = IDonaswapV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IDonaswapV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IDonaswapV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IDonaswapV3Pool(pool).setLmPool(lmPool);
    }
}
