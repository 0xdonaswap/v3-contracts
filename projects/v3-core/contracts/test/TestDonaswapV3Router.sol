...
contract TestDonaswapV3Router is IDonaswapV3SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IDonaswapV3Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function donaswapV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? IDonaswapV3Pool(msg.sender).token0()
                : IDonaswapV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IDonaswapV3Pool(pools[0]).token1();
            IDonaswapV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IDonaswapV3Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IDonaswapV3Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
