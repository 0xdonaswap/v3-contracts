...
describe('DonaswapV3Factory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: DonaswapV3PoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as DonaswapV3PoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as DonaswapV3Factory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [DonaswapV3Factory, DonaswapV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('DonaswapV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('DonaswapV3Pool')
