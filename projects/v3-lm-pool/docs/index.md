...
```solidity
function getLatestPeriodInfo(address _v3Pool) external view returns (uint256 flamePerSecond, uint256 endTime)
```
...
```solidity
contract IDonaswapV3FactoryOwner factoryOwner
```
...
```solidity
constructor(address _masterChef, contract IDonaswapV3FactoryOwner _factoryOwner) public
```

### deploy

```solidity
function deploy(contract IDonaswapV3Pool pool) external returns (contract IDonaswapV3LmPool lmPool)
```
...
| ---- | ---- | ----------- |
| pool | contract IDonaswapV3Pool | The contract address of the Donaswap V3 pool |
